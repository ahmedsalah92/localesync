...
**Tooltip on hover — `Jump to node`.** The icon alone says *there is an action here* but not which one. `Section — Jump Affordance` shows the hover state: tooltip left of the icon (so it never clips the panel's right edge), arrow pointing right at the target, 8px clear. *(Renamed 2026-09-12 from `Zoom to node`. The icon rationale above explicitly rejected `icon.24.zoom.in.small` because a magnifier reads as a viewport tool rather than a per-node action, so the tooltip was reasserting the metaphor the icon choice threw out. `jumpLabel`, this section's own name, and main's `Can't jump to that node` error copy all already said *jump*. The canvas still reads `Zoom to node` in `Section — Jump Affordance` and is owed a follow-up from Muhammed.)*
...
- ~~Window dimensions.~~ **Re-decided 2026-08-08 — resizable, `400×720` default, `340×480` minimum.** Previously "fixed 340×560, resizable post-launch." Reopened because the Panel control cluster adds a control bar (~44px as hand-built; 40px once on kit components) to a window with no spare vertical budget: at 340×560 the list would have dropped from 8 visible rows to 7, paying a row for the controls that make the product usable. Width was the harder constraint — four controls in one bar at 340px forces two into an overflow menu for reasons of geometry rather than design.
  - **Minimum `340×480` on screen (`resize(340, 440)` at the iframe)** — the old fixed size becomes the floor, so nothing already built needs redesign. 5 tabs × 68px is exactly 340, so the tab bar returns to its current geometry at minimum width.
  ...
  - **Scrollbar clearance.** The fixed-position `ResizeHandle` paints above the unpositioned scroll container and therefore hit-tests first in the 16×16 corner zone. Existing footerless-list bottom padding remains end-of-list spacing; padding moves content, not the scrollbar track, and is not the clearance mechanism.
  - **Implementation (LS-1 scaffold, not the engine — not yet written):** pointer handlers on the grip calling `figma.ui.resize()`. Three things the API will not do for you:
    1. **Clamping.** `resize()` accepts whatever it's given, so `resize(340, 440)` is only a `340×480` on-screen minimum if the handler enforces it. Unclamped, it's a suggestion.
  ...
  - **Shell framing convention.** Shells are drawn 400×720, of which only the lower **680** is the plugin. The top 40px band is Figma's own window chrome, drawn for context and labelled as such on the layer — Figma always renders it above an iframe UI and it cannot be suppressed (`showUI` exposes only `visible`, `width`, `height`, `title`, `position`, `themeColors`). The mark in that band is a stand-in: the real icon is uploaded with the Community listing (LS-16) and pulled in automatically once published, cannot be set from code, and must read on both light and dark chrome. Iframe stack: Tab Bar 40 + Content Area 640 (600 with the Applied Banner). Recorded on canvas at `435:1442`, 2026-09-04.
...
- ~~Hug-node AI-spec gap.~~ Hug nodes with a constrained ancestor measure against that ancestor's width via parent-walking and appear as normal rows. Hug nodes with no constrained ancestor show as "un-measurable" rows, same treatment as missing-font nodes. The free/paid measurement-accuracy split (deterministic JS vs. AI-enhanced) sits underneath and doesn't change the UX.
  - **Correction (2026-09-05, LS-8.2 §5 carry-forward 11).** The entry above is stale: there is no parent-walk, and hug nodes are never "un-measurable". LS-7 §2 resolved **immediate-parent-only** — a hug node is compared against its immediate parent's bounds (`parent-escape`), and a hug node whose parent is the page gets `fits` with reason `no-container`, not `unmeasurable`. That is what `src/main/overflow/measure.ts` implements and what the `hug-page-parent` row of `fixtures/overflow-spike.fig` asserts. Sibling-collision detection for hug nodes remains Phase 2 (LS-8.1 §5 carry-forward 8).
