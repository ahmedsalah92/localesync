...
| `rtl-mirror.fig` | Every flip rule and hazard in `docs/rtl-mirroring-ruleset.md` → LS-11. Carries the structures no other fixture has: a `GRID` frame with a spanning child (`setGridChildPosition` throws on transient overlap), an overlapping stack (reversing children reverses PAINT order), an instance (children cannot be reparented) and a locked row | `rtl-mirror.md` | **Generate rtl-mirror** | <https://www.figma.com/design/QqoBX6ex1AEnCAwa0ZPpz4/rtl-mirror.fig> |
...
| `preview.fig` | Apply, switch, edit, revert, storage and undo counts → LS-12. **Writes plugin data** (Extract stamps) — never reuse another fixture for it. Import files `preview/de.json` and `preview/translations.csv` sit alongside | `preview.md` | **Generate preview** | <https://www.figma.com/design/iewxV1vWLvgKDMuciYOvKn/preview.fig> |
